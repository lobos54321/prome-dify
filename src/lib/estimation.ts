...
export class UsageEstimator {
  ...
  private readonly BASE_SYSTEM_TOKENS = 50; // Base system message overhead
  private readonly CONTEXT_MULTIPLIER = 1.2; // Multiplier for conversation context
  ...
  estimateResponseTokens(context: ConversationContext): number {
  ...
  private hasCodeRequest(text: string): boolean {
  ...
  private getConfidenceLevel(context: ConversationContext): 'low' | 'medium' | 'high' {
    let confidence = 'medium' as 'low' | 'medium' | 'high';
  ...
  async getUserUsageStats(userId: string, days: number = 30): Promise<{
    ...
    const totalTokens = usageRecords.reduce((sum: number, record: any) => sum + record.tokens, 0);
